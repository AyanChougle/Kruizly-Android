...
void main() {
  group('1. Kruizly Model Serialization Tests', () {
    test(
      'VehicleModel.fromJson parses JSON correctly with computed getters',
      () {
        final json = {
          'id': 101,
          'car_id': 'CAR-101',
    ...
    test(
      'UserModel.fromJson and toJson parses user profile fields correctly',
      () {
        final json = {
          'id': 99,
    ...
    test(
      'CouponModel parses properties and CouponValidationResult maps correctly',
      () {
  ...
  group('3. Kruizly Widget Rendering Tests', () {
    testWidgets('StatusBadge renders correct labels and styling', (
      tester,
    ) async {
    ...
    testWidgets('GlassCard displays content and responds to taps', (
      tester,
    ) async {
      var tapped = false;
    ...
    testWidgets(
      'PaymentPlanSelector renders advance and full options and allows selection',
      (tester) async {
    ...
    test(
      'AdminStatsModel parses Hostinger live and effective stats with monthly ledger',
      () {
    ...
    test(
      'AdminStatsModel.empty has strictly zeroed values with no hardcoded demo data',
      () {
    ...
    testWidgets(
      'PriceBreakdownCard renders properly in both Light and Dark modes',
      (tester) async {
    ...
    testWidgets(
      'VehicleCard renders available tag, hourly rate, specifications, and action buttons',
      (tester) async {
    ...
    testWidgets(
      'BackgroundVideoWidget mounts safely and renders child content',
      (tester) async {
        await tester.pumpWidget(
          const MaterialApp(
            home: Scaffold(
              body: BackgroundVideoWidget(
                isEnabled: true,
                child: Center(child: Text('Kruizly EXECUTIVE CONTENT')),
              ),
            ),
          ),
        );
        ...
        await tester.pumpWidget(
          const MaterialApp(
            home: Scaffold(
              body: BackgroundVideoWidget(
                isEnabled: false,
                child: Center(child: Text('Kruizly STATIC CONTENT')),
              ),
            ),
          ),
        );
